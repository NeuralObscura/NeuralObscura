...
class DeconvolutionNet(chainer.Chain):
    ...
    def __call__(self, x, test=False):
        ...
        h = F.relu(self.c2(h))
        h = self.b2(h, test=test)
        h = F.relu(self.c3(h))
        h = self.b3(h, test=test)
        ...
        h = F.relu(self.d1(h))
        h = self.b4(h, test=test)
        h = F.relu(self.d2(h))
        h = self.b5(h, test=test)
        h = self.d3(h)
        h = (F.tanh(h) + 1) / 2
        return h
...
np.save(args.gtout, gt)

# Run me:
# python chainer_neuralstyle/deconv_ground_truth.py NeuralObscura/debug.png --gtout NeuralObscuraTests/testdata/deconv-ground-truth.npy --params chainer_neuralstyle/models/composition.model
